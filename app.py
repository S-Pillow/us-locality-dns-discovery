...
class DiscoveryApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        wordlist_frame = ttk.LabelFrame(main, text="Wordlist Sources", padding=8)
        ...
        for index, (label, value) in enumerate(profile_rows):
            ...
            ).grid(row=0, column=index, sticky=tk.W, padx=(0, 16), pady=2)

        scan_options_frame = ttk.LabelFrame(main, text="Scan Options", padding=8)
        scan_options_frame.pack(fill=tk.X, pady=(0, 8))

        ttk.Checkbutton(scan_options_frame, text="Attempt AXFR", variable=self.axfr_var).grid(
            row=0, column=0, sticky=tk.W, padx=(0, 20), pady=2
        )
        ttk.Checkbutton(
            scan_options_frame,
        ...
        preflight_frame = ttk.LabelFrame(main, text="Preflight Summary", padding=8)
        preflight_frame.pack(fill=tk.X, pady=(0, 8))
        ...
        progress_frame = ttk.LabelFrame(main, text="Scan Progress", padding=8)
        ...
        actions_frame = ttk.Frame(self, padding=(10, 8))
    ...
    def _add_file_row(
        ...
        entry.grid(row=row, column=1, sticky=tk.EW, padx=(8, 8), pady=3)
    ...
    def _add_output_folder_row(self, parent: ttk.Frame, row: int) -> None:
        ttk.Label(parent, text="Output folder:").grid(row=row, column=0, sticky=tk.W, pady=3)
        entry = ttk.Entry(parent, textvariable=self.output_folder_var, width=70)
        entry.grid(row=row, column=1, sticky=tk.EW, padx=(8, 8), pady=3)
        button_frame = ttk.Frame(parent)
        button_frame.grid(row=row, column=2, pady=3)
