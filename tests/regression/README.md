...
Ticket 26 chains through Ticket 25, which chains through Ticket 24, which chains
through Ticket 23, which chains through Ticket 22. Ticket 22 may invoke legacy
scripts under `output/` (for example `_ticket20_verify.py`) when those files
exist on disk. Legacy scripts are not source-controlled; durable Tickets 22–26
live here.
